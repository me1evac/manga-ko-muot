...
// src/store/kv.ts
var KEYS = {
  story: /* @__PURE__ */ __name((id) => `story:${id}`, "story"),
  storyList: "story:list",
  chapter: /* @__PURE__ */ __name((sid, cid) => `chapter:${sid}:${cid}`, "chapter"),
  chapterList: /* @__PURE__ */ __name((sid) => `chapter:list:${sid}`, "chapterList"),
  page: /* @__PURE__ */ __name((cid, num) => `page:${cid}:${num}`, "page"),
  pageList: /* @__PURE__ */ __name((cid) => `page:list:${cid}`, "pageList"),
  configPassword: "config:password",
  storyNextId: "config:storyNextId",
  chapterNextId: /* @__PURE__ */ __name((sid) => `config:chapterNextId:${sid}`, "chapterNextId")
};
...
// src/handlers/stories.ts
var app = new Hono2();
app.get("/", async (c) => {
  const kv = c.env.MANGA_KV;
  const ids = await getJson(kv, KEYS.storyList) ?? [];
  const stories = [];
  for (const id of ids) {
    const s = await getJson(kv, KEYS.story(id));
    if (s) stories.push(s);
  }
...
app.get("/:id", async (c) => {
  ...
  const story = await getJson(kv, KEYS.story(id));
  if (!story) return c.json({ error: "not found" }, 404);
  const chapterIds = await getJson(kv, KEYS.chapterList(id)) ?? [];
  const chapters = [];
  for (const cid of chapterIds) {
    const ch = await getJson(kv, KEYS.chapter(id, cid));
...
app.post("/", async (c) => {
  ...
  const existing = await getJson(kv, KEYS.story(body.id));
...
var stories_default = app;

// src/handlers/chapters.ts
var app2 = new Hono2();
app2.get("/:storyId/:chapterId", async (c) => {
  const kv = c.env.MANGA_KV;
  const { storyId, chapterId } = c.req.param();
  const chapter = await getJson(kv, KEYS.chapter(storyId, chapterId));
  if (!chapter) return c.json({ error: "not found" }, 404);
  const pageNums = await getJson(kv, KEYS.pageList(chapterId)) ?? [];
  return c.json({ chapter, pageCount: pageNums.length });
});
app2.get("/:storyId", async (c) => {
  const kv = c.env.MANGA_KV;
  const { storyId } = c.req.param();
  const ids = await getJson(kv, KEYS.chapterList(storyId)) ?? [];
  const chapters = [];
  for (const cid of ids) {
    const ch = await getJson(kv, KEYS.chapter(storyId, cid));
    if (ch) chapters.push(ch);
  }
  chapters.sort((a, b) => a.number - b.number);
...
app2.post("/", async (c) => {
  const kv = c.env.MANGA_KV;
  const body = await c.req.json();
  if (!body.storyId || !body.title || body.number === void 0) {
  ...
  const list = await getJson(kv, KEYS.chapterList(chapter.storyId)) ?? [];
  list.push(chapter.id);
  await putJson(kv, KEYS.chapterList(chapter.storyId), list);
  return c.json(chapter, 201);
});
app2.patch("/:storyId/:chapterId/reorder", async (c) => {
  const kv = c.env.MANGA_KV;
  const { storyId, chapterId } = c.req.param();
  const { newNumber } = await c.req.json();
  const chapter = await getJson(kv, KEYS.chapter(storyId, chapterId));
  if (!chapter) return c.json({ error: "not found" }, 404);
...
app2.delete("/:storyId/:chapterId", async (c) => {
  const kv = c.env.MANGA_KV;
  const { storyId, chapterId } = c.req.param();
  await kv.delete(KEYS.chapter(storyId, chapterId));
  const list = await getJson(kv, KEYS.chapterList(storyId)) ?? [];
  await putJson(kv, KEYS.chapterList(storyId), list.filter((id) => id !== chapterId));
  return c.json({ ok: true });
});
var chapters_default = app2;

// src/services/telegram.ts
async function sendPhoto(botToken, chatId, file, signal) {
...
function getFileUrl(botToken, filePath) {
...
var app3 = new Hono2();
app3.get("/list/:chapterId", async (c) => {
  const kv = c.env.MANGA_KV;
  const { chapterId } = c.req.param();
  const pageNums = await getJson(kv, KEYS.pageList(chapterId)) ?? [];
  const pages = [];
  for (const num of pageNums.sort((a, b) => a - b)) {
    const page = await getJson(kv, KEYS.page(chapterId, num));
    if (page) pages.push(page);
  }
  return c.json({ pages, chapterId });
});
app3.get("/:fileId", async (c) => {
  const kv = c.env.MANGA_KV;
...
app4.post("/", async (c) => {
  ...
  const raw2 = body["files"];
  const files = Array.isArray(raw2) ? raw2.filter((f) => f instanceof File) : raw2 instanceof File ? [raw2] : [];
  ...
  const story = await getJson(kv, KEYS.story(storyId));
  if (!story) return c.json({ error: "story not found" }, 404);
  const chapter = await getJson(kv, KEYS.chapter(storyId, chapterId));
  if (!chapter) return c.json({ error: "chapter not found" }, 404);
  const pageRecords = [];
  const existingPages = await getJson(kv, KEYS.pageList(chapterId)) ?? [];
...
app5.get("/password", async (c) => {
  const kv = c.env.MANGA_KV;
  const password = await kv.get(KEYS.configPassword);
  return c.json({ password: password ?? c.env.ADMIN_PASSWORD });
});
app5.patch("/password", async (c) => {
  const kv = c.env.MANGA_KV;
...
var app6 = new Hono2();
app6.use("*", cors());
app6.route("/api/stories", stories_default);
app6.route("/api/chapters", chapters_default);
app6.route("/api/images", pages_default);
app6.route("/api/pages", pages_default);
app6.route("/api/upload", upload_default);
app6.route("/api/config", config_default);
app6.get("/api/health", (c) => c.json({ ok: true }));
app6.all("*", (c) => c.json({ error: "not found" }, 404));
...
// .wrangler/tmp/bundle-Y44yrN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Y44yrN/middleware-loader.entry.ts
