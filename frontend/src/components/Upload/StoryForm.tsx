...
export default function StoryForm({ onSuccess }: StoryFormProps) {
  ...
  function generateRandomId(): string {
    ...
    const zeroCount = Math.floor(Math.random() * 2) + 2
    ...
    s += '0'.repeat(zeroCount)
    s += Math.floor(Math.random() * Math.pow(10, digitCount)).toString().padStart(digitCount, '0')
    return s
  }
